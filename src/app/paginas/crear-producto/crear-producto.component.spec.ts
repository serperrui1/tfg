...
describe('Crear Producto', () => {
  ...
  beforeEach(async () => {
    ...
    component.crearProductoForm = fb.group({
   ...
   describe('Formulario Crear Producto', () => {
    it('Formulario: Iniciado correctamente', () => {
      ...
      let titulo = component.crearProductoForm.controls['titulo']
      let descripcion = component.crearProductoForm.controls['descripcion']
      let categoria = component.crearProductoForm.controls['categoria']
      let unidadesMinimas = component.crearProductoForm.controls['unidadesMinimas']
      let stock = component.crearProductoForm.controls['stock']
      let precio = component.crearProductoForm.controls['precio']
      let tiempoEnvio = component.crearProductoForm.controls['tiempoEnvio']
      let subcategoria = component.crearProductoForm.controls['subcategoria']

      expect(titulo.value).toEqual('')
      expect(descripcion.value).toEqual('')
      expect(categoria.value).toEqual('Libros, Música, Vídeo y DVD')
      expect(unidadesMinimas.value).toEqual('')
      expect(stock.value).toEqual("")
      expect(precio.value).toEqual("")
      expect(tiempoEnvio.value).toEqual("")
      expect(subcategoria.value).toEqual("")


      expect(component.crearProductoForm.valid).toBeFalsy()
  ...
  it('Botón para crear el Producto', () => {
    ...
    spyOn(component, 'crearProducto')
    const button = fixture.debugElement.nativeElement.querySelector('#submit');
    button.click();
    expect(component.crearProducto).toHaveBeenCalled()
  })
})
